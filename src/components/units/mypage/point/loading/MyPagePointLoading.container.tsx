...
import { FETCH_POINT_TRANSACTIONS_OF_LOADING } from "../MyPagePoint.queries";
export default function MyPagePointLoading() {
  const { data, refetch } = useQuery(FETCH_POINT_TRANSACTIONS_OF_LOADING);
  ...
  return (
    <S.Wrapper>
      <S.TableWrapper>
        ...
        <S.TableBody>
          {data?.fetchPointTransactionsOfLoading.map((e: any, idx: number) => (
            <S.Row key={e._id}>
